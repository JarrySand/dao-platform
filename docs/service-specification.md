...
| 機能                     | 説明                                                                                                                       |
| ------------------------ | -------------------------------------------------------------------------------------------------------------------------- |
| 投票文書登録             | 議案書・投票結果・議事録を、投票トランザクションとオンチェーンで紐付けて登録する（TX ハッシュ・チェーン ID を EAS に記録） |
| トランザクション表示     | 紐付けられた投票トランザクションの情報を表示し、ブロックエクスプローラーへリンクする                                       |
| 投票文書フィルター・検索 | ドキュメント一覧で投票タイプによるフィルタリング、トランザクションハッシュによる検索を行う                                 |
...
- v1 で登録された文書の証明は、v1 スキーマで引き続き参照・検証可能
- v2 以降の新規登録は、拡張されたスキーマ（文書タイプ・投票 TX リンク等）で記録する
- 閲覧画面では v1・v2 両方の文書を統合的に表示する
- v1 スキーマに存在しないフィールド（文書タイプ等）は「不明」として表示される
...
| タイプ    | 説明           | 具体例                         |
| --------- | -------------- | ------------------------------ |
| articles  | 定款・基本規程 | 合同会社定款、組織規程         |
| meeting   | 会議・議事録   | 社員総会議事録、理事会議事録   |
| token     | トークン関連   | トークン発行規程、分配方針     |
| operation | 業務・運営     | 事業計画書、業務報告書         |
| voting    | 投票関連       | 議案書、投票結果報告、決議文書 |
| other     | その他         | 上記に該当しない文書           |

### 対応ファイル形式

PDF, DOC, DOCX, TXT, JSON（上限 10MB/ファイル）
...
| 認証方式          | 用途                       | 必要な場面                                   |
| ----------------- | -------------------------- | -------------------------------------------- |
| メール/パスワード | ログイン状態の管理         | My DAO 管理画面、ダッシュボードへのアクセス  |
| ウォレット接続    | ブロックチェーン操作の権限 | DAO 登録、文書登録・失効（署名が必要な操作） |

- 閲覧・検証・ダウンロードには認証不要
- ブロックチェーンへの書き込み（DAO 登録、文書登録、文書失効）にはウォレット接続が必須
- DAO の編集・文書管理は管理者（DAO 登録時のウォレットアドレスと一致）のみ可能
- パスワードリセット機能あり（メールによるリセットリンク送信）
